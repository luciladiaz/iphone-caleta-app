  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: 'var(--rv-surface)', border: '1px solid var(--rv-border)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 400 }}>
        ...
        {costoN > 0 && pvN > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div style={{ background: 'var(--rv-surface-alt)', borderRadius: 10, padding: '12px 14px' }}>
                <div style={{ color: 'var(--rv-text-dim)', fontSize: 10, marginBottom: 4 }}>GANANCIA USD</div>
                <div style={{ fontWeight: 800, fontSize: 18, color: 'var(--rv-text)' }}>
                  {gananciaUSD > 0 ? '+' : ''}USD {gananciaUSD.toFixed(2)}
                </div>
              </div>
              <div style={{ background: 'var(--rv-surface-alt)', borderRadius: 10, padding: '12px 14px' }}>
                <div style={{ color: 'var(--rv-text-dim)', fontSize: 10, marginBottom: 4 }}>GANANCIA ARS</div>
                <div style={{ fontWeight: 800, fontSize: 18, color: 'var(--rv-text)' }}>
                  ${gananciaARS.toLocaleString('es-AR', { maximumFractionDigits: 0 })}
                </div>
              </div>
              <div style={{ background: 'var(--rv-surface-alt)', borderRadius: 10, padding: '12px 14px', gridColumn: '1/-1' }}>
                <div style={{ color: 'var(--rv-text-dim)', fontSize: 10, marginBottom: 4 }}>PRECIO DE VENTA EN ARS</div>
                <div style={{ fontWeight: 800, fontSize: 20, color: 'var(--rv-accent)' }}>
        ...
        {(!costoN || !pvN) && (
          <div style={{ textAlign: 'center', color: 'var(--rv-text-dim)', fontSize: 13, padding: '20px 0' }}>
