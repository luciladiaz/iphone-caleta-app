    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
            ...
            <div key={p.id} style={{ background: '#1c1c1e', border: '1px solid #2c2c2e', borderRadius: 14, padding: 22 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 13 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#86868b' }}>Equipos totales</span>
                  <span style={{ fontWeight: 700 }}>{d.total || 0}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#86868b' }}>Disponibles</span>
                  <span style={{ color: '#30d158', fontWeight: 700 }}>{d.disponibles || 0}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#86868b' }}>Vendidos</span>
                  <span style={{ fontWeight: 700 }}>{d.vendidos || 0}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#86868b' }}>En consignación</span>
                  <span style={{ color: '#ff9f0a', fontWeight: 700 }}>{d.consignacion || 0}</span>
                </div>
              </div>
            </div>
          );
        ...
        {proveedores.length === 0 && (
          <div style={{ color: '#86868b', fontSize: 14 }}>No hay proveedores. Agregá desde ⚙️ Configuración.</div>
        )}
      </div>
    </div>
  );
